...
if not fork in fork_pushback_block_count.keys():
    raise Exception('Invalid fork')

def calculateDifficulty(parent_diff, parent_timestamp, parent_uncle_hash, current_timestamp, current_block, fork):
    ...
    if fork == 'Frontier':
        expected_diff = parent_diff + parent_diff // 2048 * (1 if current_timestamp - parent_timestamp < 13 else -1) + int(2**((current_block // 100000) - 2))
    elif fork == 'Homestead':
        expected_diff = parent_diff + parent_diff // 2048 * max(1 - (current_timestamp - parent_timestamp) // 10, -99) + int(2**((current_block // 100000) - 2))
    else:
        adj_factor = max((2 if parent_uncle_count else 1) - ((current_timestamp - parent_timestamp) // 9), -99)
...
for og_test in og_tests:
    if ("parentDifficulty" not in og_tests[og_test]
    ...
    (parent_diff, parent_uncles_hash, current_block_number, current_diff, parent_timestamp, current_timestamp) = (og_tests[og_test]["parentDifficulty"],
                og_tests[og_test]["parentUncles"], og_tests[og_test]["currentBlockNumber"], og_tests[og_test]["currentDifficulty"],
                og_tests[og_test]["parentTimestamp"], og_tests[og_test]["currentTimestamp"])
    ...
    new_test["parentUncles"] = "0x00" if parent_uncles_hash.lower() == emptyOmmersHash.lower() else "0x01"
    new_test["parentTimestamp"] = hex(parent_timestamp)
    ...
    if current_timestamp == parent_timestamp:
        ...
        if parent_timestamp > current_timestamp:
            raise Exception("Invalid timestamp in test case {}, parent_timestamp={}, current_timestamp={}".format(og_test,parent_timestamp,current_timestamp))

    new_test["currentTimestamp"] = hex(current_timestamp)
    new_test["currentDifficulty"] = current_diff
    
    exp_diff = calculateDifficulty(int(parent_diff, 16), parent_timestamp, parent_uncles_hash, current_timestamp, int(current_block_number, 16), fork)

    if int(current_diff, 16) != exp_diff:
        raise Exception("Invalid calculated difficulty! exp={}, act={}".format(exp_diff, current_diff))
