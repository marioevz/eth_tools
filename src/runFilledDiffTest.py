...
for test_name in filled_test.keys():
    for fork in filled_test[test_name]:
        if fork == "_info":
            continue

        for tc in filled_test[test_name][fork]:
            test_count += 1
            tc = filled_test[test_name][fork][tc]
            ...
            parent_uncle_hash = (
                nonEmptyOmmersHash if parent_uncle_count else emptyOmmersHash
            )

            adj_factor = max(
                (2 if parent_uncle_count else 1)
                - ((timestamp - parent_timestamp) // 9),
                -99,
            )

            diff_minus_bomb = (
                parent_diff + (parent_diff // BLOCK_DIFF_FACTOR) * adj_factor
            )
            ...
            if expected_diff != tc_difficulty:
                ...
                print(
                    "TC {} Fail: Expected difficulty is different from actual difficulty in test case (expected={} / test case={})".format(
                        tc, hex(expected_diff), hex(tc_difficulty)
                    )
                )
                pprint.pprint(tc)
                pprint.pprint(
            ...
            if "currentDifficulty" not in data:
            ...
            if int(data["currentDifficulty"], 16) == expected_diff:
                ...
                failing_count += 1
                print(
                    "Verification FAIL: Expected {}, Received {}".format(
                        hex(expected_diff), data["currentDifficulty"]
                    )
                )
...
if failing_count > 0 or passing_count < test_count:
    print(
        "Verification FAIL: One or more test cases failed, f={} (exp={},other={}), p={}, t={}".format(
            failing_count,
            expected_diff_count,
            failing_count - expected_diff_count,
            passing_count,
            test_count,
        )
    )
